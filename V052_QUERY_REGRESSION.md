...
- `2026年未缴养老保险、同时属于独居关爱且60岁以上的人数` → 使用 `people_governance`，养老年度、缴费状态、关爱事项、年龄全部进入计划，聚合固定为 `count_distinct(person_id)`。
- `2组、女性、2026年未缴养老保险、同时有重点转移记录的人员名单` → 使用 `people_governance` 并输出去重人员名单。
- 跨域人员视图禁止金额 `sum/avg`，金额统计必须走对应单业务数据集，避免一对多 Join 放大。
